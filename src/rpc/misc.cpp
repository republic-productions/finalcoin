...
static RPCHelpMan validateaddress()
{
    return RPCHelpMan{"validateaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The finalcoin address to validate"},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        {RPCResult::Type::BOOL, "isvalid", "If the address is valid or not"},
                        {RPCResult::Type::STR, "address", /* optional */ true, "The finalcoin address validated"},
...
static RPCHelpMan verifymessage()
{
    return RPCHelpMan{"verifymessage",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The finalcoin address to use for the signature."},
...
static RPCHelpMan echo(const std::string& name)
{
    return RPCHelpMan{name,
                ...
                "finalcoin-cli and the GUI. There is no server-side difference.",
                {
    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                // Spawn a new finalcoin-node process and call makeEcho to get a
                // client pointer to a interfaces::Echo instance running in
                // that process. This is just for testing. A slightly more
                // realistic test spawning a different executable instead of
                // the same executable would add a new finalcoin-echo executable,
                // and spawn finalcoin-echo below instead of finalcoin-node. But
                // using finalcoin-node avoids the need to build and install a
                // new executable just for this one test.
                auto init = ipc->spawnProcess("finalcoin-node");
                echo = init->makeEcho();
                ipc->addCleanup(*echo, [init = init.release()] { delete init; });
            } else {
                // IPC support is not available because this is a finalcoind
                // process not a finalcoind-node process, so just create a local
