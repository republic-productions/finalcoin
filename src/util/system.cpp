...
// For details see https://github.com/republicproductions/finalcoin/pull/22348.
...
const char * const FINALCOIN_CONF_FILENAME = "finalcoin.conf";
const char * const FINALCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/republicproductions/finalcoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //finalcoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", FINALCOIN_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "finalcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Finalcoin
    // macOS: ~/Library/Application Support/Finalcoin
    // Unix-like: ~/.finalcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Finalcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Finalcoin";
#else
    // Unix-like
    return pathRet / ".finalcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", FINALCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Finalcoin Core copyright is not removed by accident
    if (copyright_devs.find("Finalcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Finalcoin Core developers";
    }
