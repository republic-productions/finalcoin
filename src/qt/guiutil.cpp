...
#include <qt/finalcoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Finalcoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new FinalcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new FinalcoinAddressCheckValidator(parent));
}
...
bool parseFinalcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no finalcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("finalcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!FinalcoinUnits::parse(FinalcoinUnits::BTC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseFinalcoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseFinalcoinURI(uriInstance, out);
}

QString formatFinalcoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("finalcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(FinalcoinUnits::format(FinalcoinUnits::BTC, info.amount, false, FinalcoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openFinalcoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", FINALCOIN_CONF_FILENAME));
    ...
    /* Open finalcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Finalcoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Finalcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Finalcoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Finalcoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "finalcoin.desktop";
    return GetAutostartDir() / strprintf("finalcoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a finalcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Finalcoin\n";
        else
            optionFile << strprintf("Name=Finalcoin (%s)\n", chain);
