// Copyright (c) 2015-2020 The Finalcoin Core developers
...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<FinalcoinAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_finalcoin-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_finalcoin-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_finalcoin-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = FinalcoinUnits::formatWithUnit(unit, balance, false, FinalcoinUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    FinalcoinAmountField* amountInput = receiveCoinsDialog.findChild<FinalcoinAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("finalcoin:"), 2);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_finalcoin-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
