This directory contains the source code for the Finalcoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Finalcoin UI.
...
- ClientModel is used by the main application `finalcoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a finalcoin:... link from a browser).
...
* UI elements like FinalcoinAmountField, which inherit from QWidget.
* `finalcoinstrings.cpp`: automatically generated
* `finalcoinunits.(h/cpp)`: BTC / mBTC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/finalcoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `finalcoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `finalcoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `finalcoin-qt`
9. You're all set! Start developing, building, and debugging the Finalcoin Core GUI
