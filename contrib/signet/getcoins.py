...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing finalcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='finalcoin-cli', help='finalcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Finalcoin address to which the faucet should send')
...
parser.add_argument('finalcoin_cli_args', nargs='*', help='Arguments to pass on to finalcoin-cli (default: -signet)')

args = parser.parse_args()

if args.finalcoin_cli_args == []:
    args.finalcoin_cli_args = ['-signet']


def finalcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.finalcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = finalcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = finalcoin_cli(['getnewaddress', 'faucet', 'bech32'])
