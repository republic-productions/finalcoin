# Bootstrappable Finalcoin Core Builds

This directory contains the files necessary to perform bootstrappable Finalcoin
Core builds.
...
To build Finalcoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Finalcoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/finalcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/finalcoin-core/guix.sigs).
