...
Tracing (USDT) support for Finalcoin Core on Linux using. For more information on
USDT support in Finalcoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `finalcoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `finalcoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Finalcoin Core USDT documentation] on how to list available tracepoints in your
`finalcoind` binary.

[Finalcoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/finalcoind
```
...
Finalcoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/finalcoind
```
...
In a different terminal, starting Finalcoin Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/finalcoind -signet -reindex
```
