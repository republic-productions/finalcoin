# User-space, Statically Defined Tracing (USDT) for Finalcoin Core

Finalcoin Core includes statically defined tracepoints to allow for more
...
## Adding tracepoints to Finalcoin Core
...
Multiple tools can list the available tracepoints in a `finalcoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Finalcoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/finalcoind
stap net        outbound_message 0x0000000000107c05 /src/finalcoind
stap validation block_connected  0x00000000002fb10c /src/finalcoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Finalcoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/finalcoind | grep NT_STAPSDT -A 4 -B 2
