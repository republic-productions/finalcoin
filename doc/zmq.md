...
The Finalcoin Core daemon can be configured to act as a trusted "border
router", implementing the finalcoin wire protocol and relay, making
...
The ZeroMQ feature in Finalcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building finalcoind:
...
For instance:
    ...
    $ finalcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubhashblock="tcp://[::1]:28333" \
               -zmqpubrawtx=ipc:///tmp/finalcoind.tx.raw \
...
These options can also be provided in finalcoin.conf.
...
From the perspective of finalcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into finalcoind directly. Furthermore, no information is
...
using. Finalcoind appends an up-counting sequence number to each
