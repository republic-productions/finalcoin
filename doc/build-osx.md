...
This guide describes how to build finalcoind, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Finalcoin Core from source.
...
### 4. Clone Finalcoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Finalcoin Core repository to a directory.
...
It is not necessary to build wallet functionality to run `finalcoind` or  `finalcoin-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/republicproductions/finalcoin/blob/master/doc/descriptors.md), `sqlite` is required.
...
Finalcoin Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/republicproductions/finalcoin/issues/7714).
...
You can deploy a `.dmg` containing the Finalcoin Core application using `make deploy`.
...
## Building Finalcoin Core

### 1. Configuration

There are many ways to configure Finalcoin Core, here are a few common examples:
...
Run the following in your terminal to compile Finalcoin Core:
...
## Running Finalcoin Core

Finalcoin Core should now be available at `./src/finalcoind`.
If you compiled support for the GUI, it should be available at `./src/qt/finalcoin-qt`.

The first time you run `finalcoind` or `finalcoin-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Finalcoin/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Finalcoin"

touch "/Users/${USER}/Library/Application Support/Finalcoin/finalcoin.conf"
...
./src/qt/finalcoin-qt -server # Starts the finalcoin-qt server mode, allows finalcoin-cli control
```
