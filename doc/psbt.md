# PSBT Howto for Finalcoin Core

Since Finalcoin Core 0.17, an RPC interface exists for Partially Signed Finalcoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/finalcoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Finalcoin transactions that are not fully signed
...
#### Multisig with multiple Finalcoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Finalcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `finalcoin-cli` is used.
