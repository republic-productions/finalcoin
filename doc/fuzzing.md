# Fuzzing Finalcoin Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Finalcoin Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/republicproductions/finalcoin
$ cd finalcoin/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/republicproductions/finalcoin/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/republicproductions/finalcoin/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/republicproductions/finalcoin/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/republicproductions/finalcoin/tree/master/src/test/fuzz).
...
The project's collection of seed corpora is found in the [`finalcoin-core/qa-assets`](https://github.com/finalcoin-core/qa-assets) repo.

To fuzz `process_message` using the [`finalcoin-core/qa-assets`](https://github.com/finalcoin-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/finalcoin-core/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`finalcoin-core/qa-assets`](https://github.com/finalcoin-core/qa-assets) repo.

Every single pull request submitted against the Finalcoin Core repo is automatically tested against all inputs in the [`finalcoin-core/qa-assets`](https://github.com/finalcoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Finalcoin Core more robust.
...
with certain assembly code from Finalcoin Core's code. See [developer notes on sanitizers](https://github.com/republicproductions/finalcoin/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing Finalcoin Core using afl++

## Quickstart guide

To quickly get started fuzzing Finalcoin Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Finalcoin Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Finalcoin Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Finalcoin Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Finalcoin
Core without having to write any custom fuzzing harness. The `finalcoind` server
...
```sh
$ mkdir finalcoin-honggfuzz-p2p/
$ cd finalcoin-honggfuzz-p2p/
$ git clone https://github.com/republicproductions/finalcoin
$ cd finalcoin/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/finalcoind.cpp b/src/finalcoind.cpp
index 455a82e39..2faa3f80f 100644
--- a/src/finalcoind.cpp
+++ b/src/finalcoind.cpp
...
EOF
$ make -C src/ finalcoind
$ mkdir -p inputs/
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/finalcoind -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
                       -nodebuglogfile -bind=127.0.0.1:18444 -logthreadnames \
                       -debug
```

# Fuzzing Finalcoin Core using Eclipser (v1.x)

## Quickstart guide

To quickly get started fuzzing Finalcoin Core using [Eclipser v1.x](https://github.com/SoftSec-KAIST/Eclipser/tree/v1.x):
...
See [this proposed patch](https://github.com/republicproductions/finalcoin/pull/22472) for more information.
...
Finalcoin Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/finalcoin-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=finalcoin-core).
...
because Finalcoin is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_finalcoin-core/latest).
