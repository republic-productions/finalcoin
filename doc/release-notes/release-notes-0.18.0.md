Finalcoin Core version 0.18.0 is now available from:

  <https://finalcoincore.org/bin/finalcoin-core-0.18.0/>
...
`/Applications/Finalcoin-Qt` (on Mac) or `finalcoind`/`finalcoin-qt` (on
Linux).
...
recommended to use Finalcoin Core on unsupported systems.
...
10.10. Additionally, Finalcoin Core does not yet change appearance when
...
`contrib/init/finalcoind.service`, it has been changed to use
`/var/lib/finalcoind` as the data directory instead of
`~finalcoin/.finalcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/finalcoind` will
exist has enough space (check using `df -h /var/lib/finalcoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/republicproductions/finalcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://finalcoincore.org/en/contact/) the Finalcoin Core
...
The systemd init file (`contrib/init/finalcoind.service`) has been changed
to use `/var/lib/finalcoind` as the data directory instead of
`~finalcoin/.finalcoin`. This change makes Finalcoin Core more consistent with
...
`datadir` option in `/etc/finalcoin/finalcoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/finalcoin/finalcoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/republicproductions/finalcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/republicproductions/finalcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/republicproductions/finalcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/republicproductions/finalcoin/blob/master/doc/finalcoin-conf.md)
  about the `finalcoin.conf` file describes how to use it to configure
  Finalcoin Core.

- A new document introduces Finalcoin Core's BIP174 [Partially-Signed
  Finalcoin Transactions
  (PSBT)](https://github.com/republicproductions/finalcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/republicproductions/finalcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Finalcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Finalcoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/republicproductions/finalcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/republicproductions/finalcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Finalcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Finalcoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/republicproductions/finalcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/republicproductions/finalcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart finalcoind with the `-deprecatedrpc=generate`
...
- A new `finalcoin-wallet` tool is now distributed alongside Finalcoin
...
This section describes planned changes to Finalcoin Core that may affect
other Finalcoin software and services.

- Since version 0.16.0, Finalcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Finalcoin Core 0.20 (expected about
  a year after 0.18), Finalcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Finalcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Finalcoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Finalcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Finalcoin Core
...
- A sub-project of Finalcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Finalcoin Core.  See their [project
  page](https://github.com/finalcoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Finalcoin Core's own implementation, although entropy
  gathered by Finalcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Finalcoin Core a
...
- On macOS, Finalcoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `finalcoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libfinalcoin_server.a` (jnewbery)
...
- #13926 [Tools] finalcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like finalcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `finalcoin-qt -version` output formatting (hebasto)
...
- #11625 Add FinalcoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libfinalcoin_server` (laanwj)
...
- #15154 Configure: finalcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/finalcoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Finalcoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libfinalcoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in finalcoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Finalcoin Core (Christewart)
...
- #12246 Bugfix: Only run finalcoin-tx tests when finalcoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_finalcoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_finalcoin` (MarcoFalke)
...
- #14186 finalcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_finalcoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update finalcoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/finalcoin-conf.md (hebasto)
...
- #15354 Add missing `finalcoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct finalcoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support finalcoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/finalcoin/).
