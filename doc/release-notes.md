*After branching off for a major version release of Finalcoin Core, use this
...
https://github.com/finalcoin-core/finalcoin-devwiki/wiki/
...
Finalcoin Core version *version* is now available from:

  <https://finalcoincore.org/bin/finalcoin-core-*version*/>
...
using the Linux kernel, macOS 10.15+, and Windows 7 and newer.  Finalcoin
...
- A finalcoind node will no longer rumour addresses to inbound peers by default.
...
* On startup, the list of banned hosts and networks (via `setban` RPC) in
  `banlist.dat` is ignored and only `banlist.json` is considered. Finalcoin Core
...
- CLI `-addrinfo` now returns a single field for the number of `onion` addresses
  ...
  for Tor V2 addresses was removed from Finalcoin Core in 22.0. (#22544)
